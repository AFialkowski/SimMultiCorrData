...
Look at lower kurtosis boundaries:
```{r}
...
1. **Distribution 1** (*normal*): the original lower kurtosis boundary (see `Lower[[1]]$Min`) is -0.2829743.  The standardized kurtosis for the distribution (0) falls above this boundary.

1. **Distribution 2** (*t, $\Large df = 10$*): the original lower kurtosis boundary (see `Lower[[2]]$Invalid.C`) of -1.370823 has been increased to -1.050823, so that `Lower[[2]]$SkurtCorr1` = 0.32.  The standardized kurtosis for the distribution (1) falls above this boundary.

1. **Distribution 3** (*Chisq, $\Large df = 4$*): the original lower kurtosis boundary (see `Lower[[3]]$Invalid.C`) of 2.975960 has been increased to 3.00596, so that `Lower[[3]]$SkurtCorr1` = 0.03.  The standardized kurtosis for the distribution (3) is approximately equal to this boundary.  This does not cause a problem since the simulated variable has a valid power method pdf.

1. **Distribution 4** (*Beta, $\Large \alpha = 4, \beta = 2$*): the original lower kurtosis boundary (see `Lower[[4]]$Invalid.C`) of -0.3922664 has been increased to -0.3722664, so that `Lower[[4]]$SkurtCorr1` = 0.02.  The standardized kurtosis for the distribution (-0.375) falls below this boundary.  This does not cause a problem since the simulated variable has a valid power method pdf.

1. **Distribution 5** (*Gamma, $\Large \alpha = 4, \beta = 4$*): the original lower kurtosis boundary (see `Lower[[5]]$Invalid.C`) of 1.461163 has been increased to 1.501163, so that `Lower[[5]]$SkurtCorr1` = 0.04.  The standardized kurtosis for the distribution (1.5) falls below this boundary.  This does not cause a problem since the simulated variable has a valid power method pdf.
...
```
**2) Count variables**   
Simulated distributions: Note the expected means and variances are also given.
```{r}
B$summary_Poisson
B$summary_Neg_Bin
```
...
First look at the t ($\Large df = 10$) distribution (2nd continuous variable):
...
Second look at the Chisq ($\Large df = 4$) distribution (3rd continuous variable):
