...
1. `plot_sim_cdf` plots the empirical cdf Fn of simulated data (see `ggplot2::stat_ecdf`).  If `calc_cprob` = TRUE, the cumulative probability up to a user-specified y-value (delta) is calculated (see `sim_cdf_prob`) and the region on the plot is filled with a dashed horizontal line drawn at $\Large Fn(delta)$.

1. `plot_sim_ext` plots simulated continuous or count data and overlays external data (as a histogram).  Unlike in `plot_sim_theory`, the vector of external data is required.  If the user wants to plot only the simulated data, `plot_sim_theory` should be used with `overlay` = FALSE.
...
1. `plot_sim_theory` plots simulated continuous or count data and overlays data (if `overlay` = TRUE) randomly generated from a target distribution specified by name and parameters or by pdf fx (bounds set equal to bounds of simulated data).
