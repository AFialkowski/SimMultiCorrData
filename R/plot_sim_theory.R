#' @title Plot Simulated Data and Target Distribution Data by Name or Function
#'
...
#'     distribution, which is specified by name (plus up to 3 parameters) or pdf function (plus support bounds).  The simulated data is
#'     scaled and then transformed (i.e. \eqn{sim_y <- sigma * scale(sim_y) + mu}) so that it has the same mean and variance as the
#'     target distribution.  It returns a \code{\link[ggplot2]{ggplot2}} object so the user can modify as necessary.
...
#' @param target_color the line color for the target distribution (default = "dark green")
#' @param target_lty the line type for the target distribution (default = 2, dashed line)
...
plot_sim_theory <- function(sim_y, title = "Simulated Data Values",
  ...
  mu <- theory[1]
  sigma <- theory[2]
  sim_y <- sort(sigma * scale(sim_y) + mu)
  ...
  plot1 <- ggplot() + theme_bw() + ggtitle(title) +
    geom_line(data = data, aes_(x = ~x, y = ~y, colour = "Sim", lty = ~type)) +
  ...
  if (overlay == TRUE) {
    ...
    data2 <- data.frame(x = x, y = sort(y_fx),
