...
#' @param power_color the line color for the power method pdf
...
#' @param n the number of random standard normal numbers to use in generating y = p(z)
...
plot_pdf_theory <- function(c = NULL, method = c("Fleishman", "Polynomial"),
                            ...
                            Dist = c("Beta", "Chisq", "Exponential", "F",
                                     ...
                                     "Weibull"),
                            params = NULL, fx = NULL, n = 100, seed = 1234) {
  set.seed(seed)
  c <- as.numeric(c)
  z <- rnorm(n, 0, 1)
  z <- sort(z)
