...
context("Plots of simulated pdf")
...
test_that("works for continuous variable", {
  expect_is(plot_sim_pdf_theory(WP$continuous_variable[, 1],
...
test_that("works for Poisson variable", {
  expect_is(plot_sim_pdf_theory(Sim1$Poisson_variables[, 1],
    overlay = FALSE), "ggplot")
  expect_is(plot_sim_pdf_theory(Sim1$Poisson_variables[, 1], cont_var = FALSE,
    Dist = "Poisson", params = lam), "ggplot")
})

test_that("works for NB variable", {
  expect_is(plot_sim_pdf_theory(Sim1$Neg_Bin_variables[, 1],
    overlay = FALSE), "ggplot")
  expect_is(plot_sim_pdf_theory(Sim1$Neg_Bin_variables[, 1], cont_var = FALSE,
    Dist = "Negative_Binomial", params = c(size, prob)), "ggplot")
})

test_that("works for continuous variable plus external data", {
  expect_is(plot_sim_pdf_ext(WP$continuous_variable[, 1],
    ext_y = rweibull(10000, 3, 5)), "ggplot")
})

test_that("works for Poisson variable plus external data", {
  expect_is(plot_sim_pdf_ext(Sim1$Poisson_variables[, 1],
    ext_y = rpois(10000, lam)), "ggplot")
})

test_that("works for NB variable plus external data", {
  expect_is(plot_sim_pdf_ext(Sim1$Neg_Bin_variables[, 1],
